...
- (void)setContent:(NSString *)content withHeaderStyle:(DoHeaderStyle *)style
{
    ...
    if ([style isKindOfClass:[DoSectionHeaderStyle class]]) {
    ...
    else if ([style isKindOfClass:[DoCellStyle class]])
    {
        self.backgroundColor = ((DoCellStyle *)style).bgColor;
    }
...
{
    NSString *des = [NSString stringWithFormat:@"section = %ld,column = %ld",(long)self.section,(long)self.column];
    return des;
}
@end

@interface doFormScrollView()
{
...
@end
@implementation doFormScrollView
